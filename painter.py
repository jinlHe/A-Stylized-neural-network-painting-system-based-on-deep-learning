...
class PainterBase():
    ...
    def _load_checkpoint(self):
        ...
        if os.path.exists((os.path.join(
            ...
            myinfo.msg = 'loading renderer from pre-trained checkpoint...'
            myinfo.save()
            ...
            myinfo.msg = 'pre-trained renderer does not exist...'
            myinfo.save()
    ...
    def _save_stroke_params(self, v):
        ...
        myinfo = info.objects.get(id=1)
        myinfo.msg = 'saving stroke parameters...'
        myinfo.save()
    ...
    def _save_rendered_images(self):
        myinfo = info.objects.get(id=1)
        myinfo.msg = 'saving rendered images...'
        myinfo.save()
...
class Painter(PainterBase):
    ...
    def _drawing_step_states(self):
        ...
        myinfo = info.objects.get(id=1)
        myinfo.msg = 'iteration step %d, G_loss: %.5f, step_psnr: %.5f, strokes: %d / %d' % (
        self.step_id, self.G_loss.item(), acc, (self.anchor_id + 1) * self.m_grid * self.m_grid, self.max_m_strokes)
        myinfo.save()
...
class ProgressivePainter(PainterBase):
    ...
    def _drawing_step_states(self):
        acc = self._compute_acc().item()
        myinfo = info.objects.get(id=1)
        myinfo.msg = 'iteration step %d, G_loss: %.5f, step_acc: %.5f, grid_scale: %d / %d, strokes: %d / %d' % (self.step_id, self.G_loss.item(), acc,
...
class NeuralStyleTransfer(PainterBase):
    ...
    def _style_transfer_step_states(self):
        acc = self._compute_acc().item()
        myinfo = info.objects.get(id=1)
        myinfo.msg = 'running style transfer... iteration step %d, G_loss: %.5f, step_psnr: %.5f' % (self.step_id, self.G_loss.item(), acc)
        myinfo.save()
